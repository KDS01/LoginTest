...
public class CryptoRSA {
	...
	public String encrypto(String text,String publicKey) {
		...
		try {
			...
			byte[] encryptoBytes = cipher.doFinal(text.getBytes());
			encryptoText=Base64.getEncoder().encodeToString(encryptoBytes);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String decrypto(String encryptoText, String privateKey) {
		...
		try {
			...
			byte[] encryptoBytes = Base64.getDecoder().decode(encryptoText.getBytes());
			byte[] decryptoBytes = cipher.doFinal(encryptoBytes);
			text= new String(decryptoBytes);
		} catch (Exception e) {
